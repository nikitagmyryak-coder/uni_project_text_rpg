...
class Ghoul(Entity):
    def __init__(self, level):
        hp = 20 + (level * 10)
        damage = 7 + (level * 4)
        super().__init__("Ghoul", "ghoul", level, hp, damage)

class Skeleton(Entity):
    def __init__(self, level):
        hp = 25 + (level * 8)
        damage = 10 + (level * 5)
        super().__init__("Skeleton", "skeleton", level, hp, damage)
...
class Wolf(Entity):
    def __init__(self, level):
        hp = 25 + (level * 12)
        damage = 6 + (level * 4)
        super().__init__("Wolf", "wolf", level, hp, damage)
...
class Bear(Entity):
    def __init__(self, level):
        hp = 50 + (level * 20)
        damage = 12 + (level * 7)
        super().__init__("Bear", "bear", level, hp, damage)

class Rat(Entity):
    def __init__(self, level):
        hp = 10 + (level * 5)
        damage = 3 + (level * 2)
        super().__init__("Rat", "rat", level, hp, damage)